...
### Casos Confirmados Dia nas Regiões
```{r}
plotcadia <- ggplot(estados2, aes(x=datas, y=numero, fill=regioes)) +
  geom_bar(stat="identity")+
  xlab("Número de Casos Por Dia") +
  ylab("Total")+
  labs(fill = "Regiao:")+
  theme_classic()
ggplotly(plotcadia) %>%
layout(legend = list(orientation = "t", x = 0.001, y =1))
...
Óbitos e Letalidade {data-icon="fa-user-plus"}
==================
...
ggplot(estadosobitos, aes(x = reorder(estado, óbitos), y=óbitos))+
  ...
  xlab("Estados")+
  ylim(NA, max(estadosobitos$óbitos)+100)+
...
ggplot(estadosacumulado, aes(x = reorder(estado, letalidade), y=letalidade))+
  ...
  coord_flip()+
   ...
   geom_text(aes(y=med_bra, label=paste0("Média Nacional ",med_bra,"%"), x= "RS"))+
  theme_classic()
